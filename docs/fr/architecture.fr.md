...
Chaque pièce ci-dessous remonte à une panne précise, dans l'ordre où elle a fait mal :

| Ce qui a cassé                                                                  | Ce qui a été ajouté                                                                |
| ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| La montre refusait silencieusement les séances exportées                        | Les sept correctifs FIT, et les tests de régression que `.fit` n'avait jamais eus  |
| Les sorties faites apparaissaient non faites                                    | L'état du plan fusionné par-dessus le stockage navigateur                          |
| Sommeil sous-évalué jusqu'à quatre heures                                       | Extraction réécrite depuis les événements de session, plus une suite de validation |
| FC de repos figée à une constante sur 447 jours                                 | Le bon champ, et le recoupement des sources                                        |
| L'efficience « progressait » de 20 s/km à l'arrivée de l'automne                | Correction thermique sur chaque fréquence cardiaque                                |
...
efface `completed`, donc le viewer doit être rendu _après_ la réintégration des données Strava.
...
2. **La FC de repos** lue sur `restingHeartRate`, une référence glissante figée, au lieu de
   `currentDayRestingHeartRate`. Faux dans _les deux_ sources — 447 jours.
...
> **Cadrage des jours.** Une ligne datée D couvre la nuit qui se _termine_ le matin de D. Une
...
| Il réclame un **Client ID** Strava           | Réponds _« utilise le connecteur Strava déjà branché »_.                                                                                                             |
...
| Il conclut n'importe quoi sur une semaine    | Bâtir un récit sur 7 jours. Réponds _« regarde tout mon historique avant de conclure »_.                                                                             |
