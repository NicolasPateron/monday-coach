...
_Français : [README](docs/fr/README.fr.md)_
...
run, your zones, and your gear mileage — so you can simply _ask_:

> _"Compare my last week to the plan. Was my easy run heart rate too high? How many kilometres are
> on my shoes?"_
...
These are the numbers that warn you _before_ you break down, not after — and they're the reason
...
- **Strava exposes a temperature stream** (`get_activity_streams`, `temp`), so heat correction
  ...
  and many don't. What the fork adds is doing it _systematically_, with a weather lookup as
...
- **Strava exposes gear mileage**, so "how far have I run in these?" needs no fork either. What's
  added is the replacement threshold and the projection — _which week of the plan does this pair
  run out?_
...
_Screenshot from the [claude-coach repository](https://github.com/felixrieseberg/claude-coach)._
...
npm install
npm run build
npm link            # ← this is the one that matters, see below
./harness/init.sh
```
...
> ### Why `npm link` is not optional
>
> The `/coach` skill ends by running `npx claude-coach render`. Without `npm link`, npm fetches
> **the published package from the registry — which still contains all seven FIT bugs.** You would
> have cloned this fork and then bypassed every fix in it.
>
> `npm link` points the `claude-coach` command at your local build instead. Verified: rendering
> a plan through the linked binary produces `heartRate`, `wktName` and `repeatUntilStepsCmplt`,
> and zero occurrences of the broken field names.
>
> To check yours: `which claude-coach` should resolve inside your clone, not into a global npm
> cache. To undo it later: `npm unlink -g claude-coach`.
...
| **4** | **Let the skill build your plan.** claude-coach doing its job: the interview, the zones, the plan, the viewer. If it renders with `npx claude-coach`, check you ran `npm link` — otherwise you get the published build, bugs included. | 30 min |
...
> don't need it. If the skill asks, reply _"use the Strava connector already connected"_.
...
You never prompt the Monday review. If you find yourself typing _"do my weekly review"_, the
...
- **the rule that came out of all of it**: _a destination that isn't verified isn't synchronised_
